...
export function Experience() {
  ...
  const getMilestoneData = (id: string) => {
    switch (id) {
      case '01':
        return t.experience.milestones.m01;
      case '02':
        return t.experience.milestones.m02;
      case '03':
        return t.experience.milestones.m03;
      default:
        return null;
    }
  };
      ...
      <section className="pt-8 sm:pt-10 border-t border-zinc-200/80 dark:border-zinc-800/80 space-y-8 sm:space-y-10">
        ...
        <div className="space-y-10 sm:space-y-12">
          {JOURNEY_MILESTONES.map((milestone, index) => {
            const milestoneData = getMilestoneData(milestone.id);
            const categoryLabel = milestoneData?.categoryLabel || milestone.categoryLabel;
            const title = milestoneData?.title || milestone.title;
            const role = milestoneData?.role || milestone.role;
            const summary = milestoneData?.summary || milestone.summary;
            const sequence = milestoneData?.sequence || milestone.designToCodeSequence;
            const highlights = milestoneData?.highlights || milestone.highlights;

            return (
              <ScrollReveal key={milestone.id} delay={index * 0.08}>
                <article
                      ...
                      {sequence && sequence.length > 0 && (
                        <div className="space-y-1.5 pt-1">
                          <div className="text-[11px] font-mono uppercase tracking-wider text-zinc-400 dark:text-zinc-500 font-medium">
                            {t.experience.workflowSequenceTitle}
                          </div>
      ...
      <ScrollReveal delay={0.1}>
        <section className="pt-8 sm:pt-10 border-t border-zinc-200/80 dark:border-zinc-800/80 space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="space-y-1">
              <span className="text-[11px] font-mono uppercase tracking-widest text-zinc-400 dark:text-zinc-500 font-medium block">
                {t.experience.progressionTag}
              </span>
              <h3 className="text-xl sm:text-2xl font-bold tracking-tight text-zinc-950 dark:text-white">
                {t.experience.progressionTitle}
              </h3>
            </div>
            <span className="text-xs font-mono text-zinc-500 dark:text-zinc-400">
              {t.experience.progressionSubtitle}
            </span>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
            {t.experience.progressionSteps.map((step) => (
              <div
                key={step.step}
                className="group border border-zinc-200/80 dark:border-zinc-800/80 rounded-sm bg-white dark:bg-[#121215] p-5 space-y-2.5 hover:border-zinc-400 dark:hover:border-zinc-600 transition-colors"
              >
      ...
      <ScrollReveal delay={0.1}>
        <section className="pt-8 sm:pt-10 border-t border-zinc-200/80 dark:border-zinc-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 shrink-0 w-full sm:w-auto">
            ...
            >
              <span>{t.experience.exploreProjectsCta}</span>
              <span
            ...
            >
              <span>{t.experience.viewToolkitCta}</span>
              <span
                className="inline-block transition-transform duration-200 group-hover:translate-x-1"
                aria-hidden="true"
              >
            ...
            <button
              ...
              className="group inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-sm border border-zinc-200 dark:border-zinc-800 bg-transparent text-zinc-800 dark:text-zinc-200 text-sm font-sans font-medium hover:bg-zinc-100 dark:hover:bg-zinc-900 transition-colors cursor-pointer"
            >
              <span>{t.experience.getInTouchCta}</span>
              <span
                className="inline-block transition-transform duration-200 group-hover:translate-x-1"
                aria-hidden="true"
              >
